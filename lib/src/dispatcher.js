...
var Dispatcher = /** @class */ (function () {
            ...
            stateEvent in this.listeners
                ? (this.listeners[stateEvent] || []).reduce(function (prev, then) {
                    ...
                    return (__assign(__assign({}, prev), (_a = {}, _a[stateEvent] = then((stateEvent in _this.prevState &&
                        ...
                        prev[stateEvent], payload) ||
                        prev[stateEvent], _a)));
                }, this.prevState)
                : this.prevState;
        var emitResult = this.finishEmit(__assign(__assign({}, this.state), applyValue(__spreadArrays(pathElements.slice(0, pathElements.length -
        ...
        stateEvent in this.ons
            ? (this.ons[stateEvent] || []).reduce(function (prev, then) {
                ...
                return (__assign(__assign({}, prev), (_a = {}, _a[stateEvent] = then((stateEvent in _this.prevState &&
                    _this.prevState[stateEvent]) ||
                    _this.search(stateEvent + "/init") ||
                    prev[stateEvent], _this.search("" + stateEvent)) ||
                    prev[stateEvent], _a)));
            }, this.prevState)
            : this.prevState;
        return emitResult;
    ...
    Dispatcher.prototype.search = function (path) {
        ...
        return resolveObjectPath(parts.slice(0, parts.length - 1).join("/"), this.state, "/");
    };
